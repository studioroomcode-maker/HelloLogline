...
export { CORE_DESIGN_SYSTEM_PROMPT, CORE_DESIGN_REFINE_SYSTEM_PROMPT, CORE_DESIGN_HINT_PROMPT, CORE_DESIGN_LEARNING_SYSTEM_PROMPT } from "./stages/stageCoreDesign.js";

export {
  WRITERS_BLOCK_QUESTIONS_SYSTEM_PROMPT,
  WRITERS_BLOCK_SYNTHESIS_SYSTEM_PROMPT,
  WRITERS_BLOCK_DEEPER_QUESTION_SYSTEM_PROMPT,
} from "./stages/stageWritersBlock.js";

export {
  VOICE_CARD_EXTRACT_SYSTEM_PROMPT,
  serializeVoiceCardForPrompt,
} from "./stages/stageVoiceCard.js";
...
// 에피소드 시리즈 설계 프롬프트
// ─────────────────────────────────────────────
export const EPISODE_SERIES_SYSTEM_PROMPT = `당신은 K-드라마·OTT 시리즈 개발 전문 작가입니다. 주어진 로그라인과 시놉시스를 기반으로 에피소드 시리즈 구조를 설계하세요.

## 한국 드라마 구조 원칙
- **기·승·전·결 4막 감정 곡선**: 기(설레임/도입) → 승(갈등 고조) → 전(폭발/반전) → 결(여운/해소)
- **짝수화 클리프행어**: 짝수 에피소드 끝은 반드시 강한 훅(충격 폭로, 위기, 감정 절정)으로 마무리
- **미드엔딩 구조**: 전체 회차의 절반 지점에서 첫 번째 대위기 또는 감정적 절정 배치
- **회차별 감정 온도**: 각 화의 감정 강도를 낮음/중간/높음/절정으로 명시
- **K-드라마 클리프행어 패턴**: "오해의 발각", "비밀 폭로", "위험에 빠진 주인공", "예상치 못한 재회"
  ...
  "episodes": [
    {
      ...
      "emotional_temperature": "낮음(설레임) | 중간(갈등) | 높음(폭발) | 절정(위기)",
      "arc_phase": "기 | 승 | 전 | 결",
      "key_scene": "이 에피소드에서 가장 강렬한 씬 묘사 (1~2문장)",
      "cliffhanger": "다음 화로 연결되는 엔딩 훅 — 짝수화는 강한 훅 필수 (마지막 화 제외)"
    }
  ],
  "series_arc": {
    "season_want": "시즌 전체의 외적 목표",
    "mid_crisis": "중반 대위기 (전체 회차 50% 지점)",
    "midpoint": "중반부 감정적 반전 또는 전환점",
    "dark_night": "주인공이 가장 낮은 지점에 떨어지는 순간",
    "finale": "시즌 피날레 방향성과 여운"
  }
}

각 에피소드는 독립된 갈등을 가지면서도 시즌 전체 아크에 기여해야 합니다.
4부작: 각 화가 기·승·전·결에 정확히 대응. 6·8부작: 기(1~2화)·승(3~4화)·전(5~6화)·결(7~8화).
12부작: 3화 단위 4막. 16부작: 4화 단위 4막, 8화와 12화에 서브 클라이맥스 배치.`;
