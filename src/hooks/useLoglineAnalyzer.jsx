// Main business-logic hook for LoglineAnalyzer
// All state, effects, handlers, and helpers extracted from the original component.
// Import paths use ../ since this file lives in src/hooks/.
...
// Stage content lazy components are now handled by StageRouter
...
/* ─── Main hook ─── */
export function useLoglineAnalyzer() {
  // ── Theme ──
  ...
  useEffect(() => {
    ...
    if (paymentKey && orderId && amount) {
      ...
      fetch("/api/subscribe/billing-auth", {
        ...
        .catch(() => showToast("error", "구독 개시 요청에 실패했어요. 카드 정보를 확인하고 다시 시도해 주세요. 카드에 청구된 흔적이 있다면 고객센터로 문의해 주세요."))
        .finally(() => setCreditPurchasing(false));
    } else if (paymentFail) {
      window.history.replaceState({}, "", window.location.pathname);
...
반드시 아래 JSON 형식으로만 응답하세요:
{
  ...
  return {
    // Theme / mode
    darkMode, setDarkMode, eduMode, setEduMode,
    // API Key
    apiKey, setApiKey, showApiKeyModal, setShowApiKeyModal, serverHasKey, setServerHasKey,
    saveApiKey,
    // Input
    logline, setLogline, genre, setGenre,
    // Stage 1 (from useStage1)
    result, setResult, loading, setLoading, error, setError,
    compareMode, setCompareMode, logline2, setLogline2, result2, setResult2, loading2, setLoading2,
    earlyCoverageResult, setEarlyCoverageResult, earlyCoverageLoading, earlyCoverageError,
    storyFixes, setStoryFixes, storyPivots, setStoryPivots, aiImprovement, setAiImprovement,
    insightResult, setInsightResult, insightLoading, insightError,
    // History
    history, setHistory, showHistory, setShowHistory,
    // Mobile
    isMobile, setIsMobile,
    // Stage
    currentStage, setCurrentStage,
    // Synopsis
    showSynopsisPanel, setShowSynopsisPanel, synopsisMode, setSynopsisMode,
    selectedDuration, setSelectedDuration, customTheme, setCustomTheme,
    customDurationText, setCustomDurationText, customFormatLabel, setCustomFormatLabel,
    // Reference scenario
    referenceScenario, setReferenceScenario,
    referenceScenarioEnabled, setReferenceScenarioEnabled,
    referenceScenarioSummary, setReferenceScenarioSummary,
    extractLoglineLoading, extractLoglineError,
    summarizeLoading, summarizeError,
    extractLoglineFromScenario, summarizeReferenceScenario,
    // Story Doctor
    showStoryDoctor, setShowStoryDoctor,
    // Framework & synopsis config
    selectedFramework, setSelectedFramework, frameworkInfoId, setFrameworkInfoId,
    directionCount, setDirectionCount, synopsisLoading, setSynopsisLoading,
    synopsisResults, setSynopsisResults, synopsisError, setSynopsisError,
    selectedSynopsisIndex, setSelectedSynopsisIndex,
    // Pipeline
    pipelineResult, setPipelineResult, pipelineFeedback, setPipelineFeedback,
    pipelineRefineLoading, pipelineEditMode, setPipelineEditMode,
    ...
    analyzeValueCharge,
    // Shadow
    shadowResult, setShadowResult, shadowLoading, shadowError, shadowRef,
    analyzeShadow,
    // Authenticity
    authenticityResult, setAuthenticityResult, authenticityLoading, authenticityError, authenticityRef,
    analyzeAuthenticity,
    // Academic
    academicResult, setAcademicResult, academicLoading, academicError,
    analyzeAcademic,
    // Beat Sheet
    beatSheetResult, setBeatSheetResult, beatSheetLoading, beatSheetError, beatSheetRef,
    beatScenes, setBeatScenes, generatingBeat, setGeneratingBeat,
    expandedBeats, setExpandedBeats, allScenesLoading, allScenesProgress,
    generateBeatSheet, generateScene, generateAllScenes,
    // Team
    teamMembers, setTeamMembers, sceneAssignments, setSceneAssignments,
    stageComments, setStageComments, currentWorkingAs, setCurrentWorkingAs,
    ...
    // Subtext
    subtextResult, setSubtextResult, subtextLoading, subtextError, subtextRef,
    analyzeSubtext,
    // Myth Map
    mythMapResult, setMythMapResult, mythMapLoading, mythMapError,
    analyzeMythMap,
    // Barthes
    barthesCodeResult, setBarthesCodeResult, barthesCodeLoading, barthesCodeError,
    analyzeBarthesCode,
    // Korean Myth
    koreanMythResult, setKoreanMythResult, koreanMythLoading, koreanMythError,
    analyzeKoreanMyth,
    // Script Coverage
    scriptCoverageResult, setScriptCoverageResult, scriptCoverageLoading, scriptCoverageError, scriptCoverageRef,
    analyzeScriptCoverage,
    // AI Transition guides
    ...
    // Comparable
    comparableResult, setComparableResult, comparableLoading, comparableError, comparableRef,
    analyzeComparableWorks,
    // Master Report
    masterReportResult, setMasterReportResult, masterReportLoading, masterReportError,
    generateMasterReport,
    // Episode
    episodeDesignResult, setEpisodeDesignResult, episodeDesignLoading, episodeDesignError,
    generateEpisodeDesign,
    // Valuation
    valuationResult, setValuationResult, valuationLoading, valuationError, valuationRef,
    analyzeValuation,
    // Dialogue Dev
    dialogueDevResult, setDialogueDevResult, dialogueDevLoading, dialogueDevError, dialogueDevRef,
    analyzeDialogueDev,
    // Structure
    structureResult, setStructureResult, structureLoading, structureError, structureRef,
    analyzeStructure,
    // Theme
    themeResult, setThemeResult, themeLoading, themeError,
    analyzeTheme,
    // Scene List
    sceneListResult, setSceneListResult, sceneListLoading, sceneListError, sceneListRef,
    generateSceneList,
    // Scenario Draft
    ...
    // Ctx badges
    treatmentCtx, setTreatmentCtx, beatSheetCtx, setBeatSheetCtx, scenarioDraftCtx, setScenarioDraftCtx,
    // Feedback refine
    treatmentFeedback, setTreatmentFeedback, treatmentRefineLoading,
    scenarioDraftFeedback, setScenarioDraftFeedback, scenarioDraftRefineLoading,
    ...
    beatSheetFeedback, setBeatSheetFeedback, beatSheetRefineLoading,
    beatSheetBefore, showBeatSheetBefore, setShowBeatSheetBefore,
    charDevFeedback, setCharDevFeedback, charDevRefineLoading,
    refineTreatment, refineBeatSheet, refineScenarioDraft,
    // History
    treatmentHistory, setTreatmentHistory, beatSheetHistory, setBeatSheetHistory,
    scenarioDraftHistory, setScenarioDraftHistory, charDevHistory, setCharDevHistory,
    pipelineHistory, setPipelineHistory,
    // Staleness
    treatmentStale, setTreatmentStale, beatSheetStale, setBeatSheetStale,
    scenarioDraftStale, setScenarioDraftStale,
    // Rewrite
    rewriteDiagResult, setRewriteDiagResult, rewriteDiagLoading, rewriteDiagError,
    partialRewriteInstruction, setPartialRewriteInstruction,
    partialRewriteResult, setPartialRewriteResult, partialRewriteLoading, partialRewriteError,
    fullRewriteNotes, setFullRewriteNotes,
    fullRewriteResult, setFullRewriteResult, fullRewriteLoading, fullRewriteError,
    ...
    editingSynopsis, setEditingSynopsis, synopsisEditDraft, setSynopsisEditDraft,
    editingBeats, setEditingBeats, beatEditDrafts, setBeatEditDrafts,
    // Treatment form
    showTreatmentPanel, setShowTreatmentPanel,
    treatmentChars, setTreatmentChars, showManualCharInput, setShowManualCharInput,
    treatmentStructure, setTreatmentStructure,
    treatmentResult, setTreatmentResult, treatmentLoading, treatmentError, treatmentRef,
    generateTreatment,
    // Project persistence
    showProjects, setShowProjects, showStoryBible, setShowStoryBible,
    savedProjects, setSavedProjects, currentProjectId, setCurrentProjectId,
    saveStatus, setSaveStatus, confirmModal, setConfirmModal, importFileRef,
    openProjects, loadProjectState, deleteProjectById, loadProjectById,
    exportProjectJson, importProjectJson,
    startNewProject,
    // First visit / welcome
    isFirstVisit, showWelcome, setShowWelcome,
    pdfLoading, setPdfLoading, shareLinkLoading, setShareLinkLoading,
    applyExampleLogline, dismissFirstVisit,
    // Demo
    isDemoMode, setIsDemoMode, demoTourStep, setDemoTourStep,
    activateDemo, deactivateDemo,
    // Reverse entry
    reverseEntryStage, setReverseEntryStage, onReverseImport,
    // Recovery banner
    showRecoveryBanner, setShowRecoveryBanner,
    // Auth
    user, setUser, authLoading, authError,
    showAdminPanel, setShowAdminPanel,
    showTeamPanel, setShowTeamPanel,
    showExportMenu, setShowExportMenu, showUserMenu, setShowUserMenu,
    ...
    subCancelling, setSubCancelling,
    // Toasts
    ...
    // Refs
    abortControllersRef,
    resultRef, synopsisRef, academicRef, mainContentRef, stageRefs,
    // Helpers
    makeController, clearController, cancelOperation, advanceToStage,
    saveToHistory, getDurText, getCustomContext,
    buildStoryDoctorContext, getStoryBible, buildUserMsg,
    decodeHtmlEntities, pushHistory, undoHistory,
    getStageStatus, getStageDoneCount, STAGE_TOTALS, statusDotColor,
    // Tier helpers
    tier, isAdmin, isPro, isBlocked, hasOwnApiKey, canUseAllStages, cc,
    TIER_LABEL, TIER_COLOR,
    // Score calculations
    structureTotal, expressionTotal, technicalTotal, qualityScore, interestScore,
    structureTotal2, expressionTotal2, technicalTotal2, qualityScore2, interestScore2,
    radarData, tabs, charCount,
    // Composite
    narrativeTheoryDone, narrativeTheoryLoading,
    charAllDone, charAllLoading, structureAllDone, structureAllLoading,
    analyzeNarrativeTheory, analyzeCharacterAll, analyzeStructureAll,
    analyzeEarlyCoverage, generateInsight, analyze, generateSynopsis,
    // Team permission helpers
    currentWorkingMember, isOwner, isReadOnly, getEditPermission, addActivity,
    startNewRevision,
    // Stage result summary
    stageResultSummary, shareSnapshot,
    // ctx value
    loglineCtxValue,
    // PDF exports
    handleExportPdf, handleShare,
    openApplicationDoc, openPitchDeck, openStoryBibleDoc,
    // isAnyLoading
    isAnyLoading,
    // ErrorMsg component
    ErrorMsg,
    // Module-level constants made available for children via spread
    GENRE_BEAT_HINTS,
  };
}

export { STAGES, GENRE_BEAT_HINTS };
export { StoryDoctorPanel, DemoTour };
