...
[극단의 부채 원칙] 후보들은 "비슷한 이야기의 다른 톤"이 아니라 **서로 다른 축의 극단**이어야 합니다. 밋밋한 아이디어를 대담하게 만드는 것은 불가능하지만, 과감한 아이디어를 작가가 길들이는 것은 쉽습니다. 각 후보는 다음 축들 중 하나를 끝까지 밀어붙이세요 (모든 후보가 같은 축이면 안 됨):
- 시점의 극단: 적대자가 옳은 버전 / 조연의 눈으로 본 버전
- 구조의 극단: 결말에서 역산한 버전 / 시간을 해체한 버전
- 톤의 극단: 장르를 뒤집은 버전 (스릴러 소재를 블랙코미디로 등)
- 수위의 극단: 가장 잔인하게 정직한 버전 / 감정을 끝까지 절제한 버전

[첫 답 금지] 각 방향을 구상할 때 가장 먼저 떠오르는 전개는 통계적 평균이므로 버리세요. 그것을 버린 뒤에 남는 선택으로 시놉시스를 쓰세요. "따뜻하다/잔잔하다/치유"로 요약되는 방향이 여럿이면 실패입니다.

시놉시스는 실제 제작 가능한 수준으로 구체적이어야 하며, 장면·감정·갈등이 생생하게 느껴져야 합니다. 극단은 "자극적"이 아니라 "구체적이고 예측 불가능"을 뜻합니다 — 반전을 위한 반전은 금지.
