/**
 * Consolidated subscription endpoint. Dispatched by ?action=xxx:
 *   /api/subscribe?action=billing-auth  — POST  — billingKey 발급 + 첫 결제
 *   /api/subscribe?action=cancel        — POST  — 구독 취소
...
// ── billing-auth ─────────────────────────────────────────────────────────
async function handleBillingAuth(req, res, email) {
...
export default async function handler(req, res) {
  ...
  let email;
  try { email = verifyToken(token).email; }
  catch { return res.status(401).json({ error: "인증 토큰이 유효하지 않습니다." }); }

  const action = req.query?.action || "";
  switch (action) {
    case "billing-auth": return handleBillingAuth(req, res, email);
    case "cancel":       return handleCancel(req, res, email);
    case "status":       return handleStatus(req, res, email);
    default:             return res.status(400).json({ error: "Invalid action. Use ?action=billing-auth|cancel|status" });
  }
}
